...
BASE_URL = "https://ai4trade.ai/discussions"
API_BASE_URL = "https://ai4trade.ai/api"
BACKUP_FILE = "discussions_backup.json"
SHEET_TEMPLATE_FILE = "discussions_sheet_template.csv"
SHEET_DATA_FILE = "discussions_sheet_data.csv"
SPREADSHEET_URL = os.getenv(
    "SPREADSHEET_URL",
    "https://docs.google.com/spreadsheets/d/1x1YiJdk45GZHytiSx5MRE96Yu7BEBUm7AXsZMmVVyBM/edit?usp=sharing",
).strip()
SHEET_NAME = os.getenv("SHEET_NAME", "discuss").strip() or "discuss"
TARGET_GID = int(os.getenv("TARGET_GID", "1799547230"))
CREDENTIALS_FILE = (
...
PAGE_SIZE = 20
DISCUSS_SCRAPE_MODE = os.getenv("DISCUSS_SCRAPE_MODE", "api").strip().lower()
...
def click_newest_tab(driver: webdriver.Chrome) -> bool:
    ...
    if newest_button is None:
        try:
            newest_button = driver.execute_script(
                """
                const buttons = Array.from(document.querySelectorAll('button'));
                return buttons.find(btn => (btn.innerText || '').trim().toLowerCase() === 'newest') || null;
                """
            )
        except Exception:
            newest_button = None
    ...
    if newest_button is None:
        print("Tombol Newest tidak ditemukan.")
        return False

    try:
        driver.execute_script("arguments[0].scrollIntoView({block:'center'});", newest_button)
    except Exception:
        pass
    ...
    try:
        ...
        try:
            ...
            print("Gagal klik tombol Newest.")
            return False
    ...
    print("Tab Newest berhasil dipilih.")
    return True
...
def scrape_all_pages_via_api() -> List[dict]:
    ...
    for page_no in range(1, MAX_PAGES_TO_SCRAPE + 1):
        ...
        try:
            ...
            print(f"Gagal ambil API discussions halaman {page_no}: {err}")
            break
        except Exception as err:
            print(f"Gagal parse API discussions halaman {page_no}: {err}")
            break

        signals = payload.get("signals") or []
        print(f"Halaman API {page_no}: ditemukan {len(signals)} diskusi")
...
def write_to_google_sheets(
    ...
    try:
        ...
        try:
            ...
            try:
                worksheet = spreadsheet.add_worksheet(title=sheet_name, rows=2000, cols=10)
                print(f"Worksheet '{sheet_name}' berhasil dibuat.")
            except Exception as err:
                print(f"Gagal membuat worksheet '{sheet_name}': {err}")
        except Exception as err:
            print(f"Gagal ambil worksheet '{sheet_name}': {err}")

        if worksheet is None and target_gid > 0:
            try:
                worksheet = spreadsheet.get_worksheet_by_id(target_gid)
                print(f"Menggunakan fallback worksheet gid={target_gid}: '{worksheet.title}'.")
            except Exception as err:
        ...
        if worksheet is None:
            ...
            if worksheet is None:
                print("Tidak ada worksheet yang bisa dipakai di spreadsheet tujuan.")
                return False
            print(
                f"Menggunakan worksheet pertama sebagai fallback: '{worksheet.title}'."
            )
...
def main() -> None:
    ...
    payload = {
        "source": "ai4trade_discussions",
        "scraped_at": datetime.now().isoformat(timespec="seconds"),
        "total": len(discussions),
        "data": discussions,
    }

    print(f"Jumlah diskusi berhasil diambil: {len(discussions)}")
